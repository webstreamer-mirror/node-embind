...
namespace emscripten {
	namespace internal {
			...
			GenericFunction function)
		{
			...
			f->name = name;
			f->argc = argCount -1;
			typedef napi_value (*Fn)(const function_t*, const napi_context_t&);
			#if _MSVC_VER
			f->invoke = static_cast<napi_value(*)(const function_t*, const napi_context_t&)>(invoker);
			#else
			f->invoke = (Fn)invoker;
			#endif
			f->function = function;
			...
			GenericFunction function)
		{
			...
			typedef class_ptr (* Fn) (const constructor_t*, const napi_context_t&);
			...
			void* setterContext)
		{
			...
			typedef napi_value(* Setter)(const property_t*, const napi_context_t&);
			prop->setter = (Setter)setter;
			prop->setter_context = setterContext;

			typedef napi_value(* Getter)(const property_t*, const napi_context_t&);
